<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Purchase Bills" :description="$bills->total().' bill(s)'" />
    </x-slot>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-ink-100 dark:divide-ink-800">
                    <thead class="bg-ink-50/80 dark:bg-ink-800/80">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-ink-500 dark:text-ink-400 uppercase tracking-wide">Bill #</th>
                        ...
                        @forelse ($bills as $bill)
                            <tr class="hover:bg-ink-50/60 dark:hover:bg-ink-800/60">
                                <td class="px-4 py-3.5 text-sm font-medium text-ink-900 dark:text-ink-50 whitespace-nowrap">
                                    <a href="{{ route('purchase-bills.show', $bill) }}" class="text-brand-600 dark:text-brand-400 hover:underline">{{ $bill->bill_number }}</a>
                                </td>
                                <td class="px-4 py-3.5 text-sm text-ink-700 dark:text-ink-200">{{ $bill->party->name }}</td>
                                <td class="px-4 py-3.5 text-sm text-ink-600 dark:text-ink-300">{{ $bill->bill_date->format('d-M-Y') }}</td>
                                <td class="px-4 py-3.5 text-sm text-right text-ink-600 dark:text-ink-300">{{ number_format($bill->total_amount, 2) }}</td>
                                <td class="px-4 py-3.5 text-sm text-right {{ $bill->amountDue() > 0 ? 'text-rose-600 dark:text-rose-400 font-medium' : 'text-ink-600 dark:text-ink-300' }}">{{ number_format($bill->amountDue(), 2) }}</td>
                                <td class="px-4 py-3.5 text-sm">
                                    <x-ui.badge :variant="match($bill->status) { 'Posted' => 'success', 'Cancelled' => 'danger', default => 'neutral' }">{{ $bill->status }}</x-ui.badge>
                                </td>
                                <td class="px-4 py-3.5 text-right text-sm">
                                    <a href="{{ route('purchase-bills.show', $bill) }}" class="text-brand-600 dark:text-brand-400 hover:text-brand-800 dark:hover:text-brand-300 font-medium">View</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7">
                                    <x-ui.empty-state title="No purchase bills yet" description="Create a bill from a completed goods receipt." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                <div class="px-4 py-4">{{ $bills->links() }}</div>
            </div>
        </x-ui.card>
    </div>
</x-app-layout>
