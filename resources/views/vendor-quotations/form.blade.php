<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header :title="$quotation ? 'Edit Quotation '.$quotation->quotation_number : 'New Vendor Quotation'" />
    </x-slot>
    ...
    <div class="max-w-5xl space-y-4">
        ...
        <x-ui.card>
            <form method="POST"
                  action="{{ $quotation ? route('vendor-quotations.update', $quotation) : route('vendor-quotations.store') }}"
                  class="space-y-8"
                  x-data="{
                      products: @js($products->map(fn ($product) => ['id' => $product->id, 'label' => $product->name.' ('.$product->sku.')'])),
                      items: @js($quotation
                          ? $quotation->items->map(fn ($item) => ['product_id' => $item->product_id, 'quantity' => (float) $item->quantity, 'unit_price' => (float) $item->unit_price, 'notes' => $item->notes])
                ...
                <x-ui.section title="Quotation Details">
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select id="vendor_id" name="vendor_id" class="mt-1 block w-full bg-white dark:bg-ink-800 text-ink-900 dark:text-ink-100 border-ink-300 dark:border-ink-600 rounded-lg shadow-sm text-sm focus:border-brand-500 focus:ring-brand-500" required>
                                ...
                                @foreach ($vendors as $vendor)
                                    <option value="{{ $vendor->id }}" @selected((string) old('vendor_id', $quotation?->vendor_id) === (string) $vendor->id)>{{ $vendor->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="quotation_date" name="quotation_date" type="date" class="mt-1 block w-full" :value="old('quotation_date', $quotation?->quotation_date?->format('Y-m-d') ?? date('Y-m-d'))" required />
                            <x-input-error :messages="$errors->get('quotation_date')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="validity_date" value="Valid Until" />
                            <x-text-input id="validity_date" name="validity_date" type="date" class="mt-1 block w-full" :value="old('validity_date', $quotation?->validity_date?->format('Y-m-d'))" />
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full bg-white dark:bg-ink-800 text-ink-900 dark:text-ink-100 border-ink-300 dark:border-ink-600 rounded-lg shadow-sm text-sm focus:border-brand-500 focus:ring-brand-500">{{ old('notes', $quotation?->notes) }}</textarea>
                </div>

                <div class="flex items-center gap-4 border-t border-ink-100 dark:border-ink-800 pt-6">
                    <x-primary-button>{{ $quotation ? __('Update Quotation') : __('Save as Draft') }}</x-primary-button>
                    <a href="{{ $quotation ? route('vendor-quotations.show', $quotation) : route('vendor-quotations.index') }}" class="text-sm text-ink-500 dark:text-ink-400 hover:text-ink-700 dark:hover:text-ink-200">Cancel</a>
                </div>
            </form>
        </x-ui.card>
    </div>
