<x-app-layout>
    ...
    <div class="space-y-4">
        ...
        <div class="flex justify-between items-center">
            <a href="{{ route('expense-categories.index') }}" class="text-sm text-brand-600 hover:text-brand-800 font-medium">&larr; Categories</a>
                    ...
                    <tbody class="divide-y divide-ink-100">
                        @forelse ($subCategories as $subCategory)
                            <tr class="hover:bg-ink-50/60">
                                <td class="px-4 py-3.5 text-sm font-medium text-ink-900">{{ $subCategory->name }}</td>
                                <td class="px-4 py-3.5 text-sm text-ink-600">{{ $subCategory->category->name }}</td>
                                ...
                                <td class="px-4 py-3.5 text-right text-sm">
                                    <a href="{{ route('expense-sub-categories.edit', $subCategory) }}" class="text-brand-600 hover:text-brand-800 font-medium">Edit</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
