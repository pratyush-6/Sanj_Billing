<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header :title="$party->name">
            <x-slot name="actions">
                <a href="{{ route('parties.ledger', $party) }}">
                    <x-secondary-button>Ledger</x-secondary-button>
                </a>
                <a href="{{ route('parties.edit', $party) }}">
                    <x-secondary-button>Edit Party</x-secondary-button>
                </a>
            </x-slot>
        </x-ui.page-header>
    </x-slot>
    ...
    <div class="space-y-4">
        ...
        <x-ui.card>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div><p class="text-ink-400 dark:text-ink-500 text-xs uppercase tracking-wide">Contact</p><p class="font-medium text-ink-900 dark:text-ink-50 mt-0.5">{{ $party->contact_person ?? '—' }}</p></div>
                <div><p class="text-ink-400 dark:text-ink-500 text-xs uppercase tracking-wide">Mobile</p><p class="font-medium text-ink-900 dark:text-ink-50 mt-0.5">{{ $party->mobile ?? '—' }}</p></div>
                <div><p class="text-ink-400 dark:text-ink-500 text-xs uppercase tracking-wide">GSTIN</p><p class="font-medium text-ink-900 dark:text-ink-50 mt-0.5">{{ $party->gstin ?? '—' }}</p></div>
                <div><p class="text-ink-400 dark:text-ink-500 text-xs uppercase tracking-wide">PAN</p><p class="font-medium text-ink-900 dark:text-ink-50 mt-0.5">{{ $party->pan ?? '—' }}</p></div>
            ...
            <x-ui.card :padded="false">
                ...
                <div class="overflow-x-auto">
                    ...
                    @foreach ($vendorProducts as $vendorProduct)
                        <form method="POST" action="{{ route('parties.products.update', [$party, $vendorProduct]) }}" id="vp-form-{{ $vendorProduct->id }}">
                            @csrf
                            @method('PUT')
                        </form>
                    @endforeach
                ...
                <table class="min-w-full divide-y divide-ink-100 dark:divide-ink-800">
                    ...
                    <tbody class="divide-y divide-ink-100 dark:divide-ink-800">
                        ...
                        @empty
                            <tr>
                                <td colspan="5">
                                    <x-ui.empty-state title="No expenses recorded for this party yet" />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
