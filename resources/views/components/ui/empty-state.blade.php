...
<div {{ $attributes->class(['flex flex-col items-center justify-center text-center py-14 px-6']) }}>
    <div class="flex h-12 w-12 items-center justify-center rounded-full bg-ink-100 text-ink-400 mb-4">
    ...
    @if ($description)
        <p class="mt-1 text-sm text-ink-400 max-w-sm">{{ $description }}</p>
    @endif
    @isset($action)
